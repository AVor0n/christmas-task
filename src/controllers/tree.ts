...
function initSnowfall() {
  const container = $<HTMLElement>('.content');
  let snowIntervalId = LS.getItem<number>('snowfall');
  ...
  const snowBtn = $<HTMLButtonElement>('.btn--snow');
  ...
  snowBtn.addEventListener('click', () => {
    if (snowIntervalId) {
      ...
      LS.setItem('snowfall', snowIntervalId);
    }
  });
}
