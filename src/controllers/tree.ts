...
function initToys() {
  const toysOnTree: ToyPostion[] = JSON.parse(localStorage.getItem('toysOnTree')) || [];
  const boxOfToys: BoxOfToys = JSON.parse(localStorage.getItem('favoriteToys')) || [];

  setToys(toysOnTree, boxOfToys);
}

function setToys(toysOnTree: ToyPostion[], boxOfToys: BoxOfToys) {
  ...
  for (const [i, boxOfToy] of boxOfToys.entries()) {
    ...
    if (boxOfToy.count > 0) toyCountLabel.style.visibility = 'visible';
  }
...
function dragAndDrop(toyEl: HTMLImageElement, event: MouseEvent) {
  ...
  function onMouseMove(e: MouseEvent) {
    ...
    treeImg.style.filter =
      elemUnderToy === treeArea ? 'drop-shadow(0 0 7px green)' : 'drop-shadow(0 0 7px red)';
  }

  //* * ------------ Drop ------------------ */

  toy.addEventListener('mouseup', (e) => {
    toy.style.display = 'none';
    const elemUnderToy = document.elementFromPoint(e.x - shiftPivotX, e.y - shiftPivotY);
    toy.style.display = '';
...
function saveToysPosition() {
  ...
  const toysOnTreeEl: HTMLImageElement[] = [...tree.querySelectorAll('.toy-image')];
  const toysOnTreeData: ToyPostion[] = [];
